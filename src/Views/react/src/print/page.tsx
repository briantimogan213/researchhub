...
export default function Print() {
  ...
  const thesisJournal = React.useMemo(() => {
    let data: any[] = pageData?.thesis || pageData?.journal || []
    if (!!department) {
  ...
  React.useEffect(() => {
    ...
    if (!pageData.thesis && !pageData.journal) {
      setError(true)
    }
  }, [])
      ...
      <div style={{ maxWidth: 8.5 * 96, minHeight: 11 * 96, backgroundColor: "white" }} className="border shadow mx-auto p-[12.2mm]">
        ...
        <div>
          ...
          <table className="table w-full border-collapse">
            ...
            <tbody>
              {thesisJournal.length === 0 && (
                <tr>
                  <td colSpan={6} className="border border-black p-4 text-center">No records</td>
                </tr>
              )}
              {thesisJournal.length > 0 && thesisJournal.map((data: any, i: number) => (
                <tr key={data.id}>
