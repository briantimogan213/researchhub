...
import { SMCCLogo } from "../global/smcclogo";
...
export default function StudentSignup() {
  ...
  const departmentCourses = React.useMemo(() => departmentsAndCourses[department] || [], [department]);
      ...
      .then(({ error, exists }) => {
        if (error) {
          ...
          if (!exists) {
            ...
            window.location.replace(pathname(`/student/signup?student_id=${studentId}&full_name=${studentName}`));
          } else {
            Sweetalert2.fire({
              icon: 'error',
              title: 'Error',
              ...
              Sweetalert2.fire({
                ...
                if (isConfirmed) {
                  window.location.replace(pathname(`/student/login?student_id=${studentId}`));
                }
              })
            });
          }
    ...
    .then(({ error, success }) => {
      if (error) {
        ...
        Sweetalert2.fire({
          ...
          window.location.href = pathname('/student/login');
        });
      }
    })
    .catch((e) => {
    ...
    <div className="relative w-full min-h-screen flex justify-center items-start">
      ...
      <div style={{background: "transparent", boxShadow: "none", minWidth: "400px", maxWidth: "400px"}}>
        ...
        <div className="bg-[#192F5D] text-white rounded-t text-center p-4">
          <h4 className="flex justify-center">STUDENT REGISTRATION</h4>
        </div>
        <div className="w-full min-h-[300px]" style={{background: "#ffffff", boxShadow: "0 5px 15px rgba(0,0,0,.05)", padding: "20px 40px 15px"}}>
          <div className="w-full">
            ...
            <div className="mt-8 text-center font-bold">
              ...
              <br />
              Already registered? <a href={pathname("/student/login")} className="text-sky-500 hover:text-sky-300 hover:underline mt-2">Login</a>
            </div>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={onSubmit} className="max-w-md mx-auto flex flex-col gap-8 border border-sky-300 rounded-lg p-8 shadow-lg pt-10">
          ...
          <div className="flex justify-center px-4">
            <select value={department} onChange={(e: any) => { setDepartment(e.target.value); setCourse(""); }} title="Department" className="p-4 w-full border-2 border-gray-300 rounded-lg bg-sky-50" required>
              ...
              {
                Object.keys(departmentsAndCourses).map((value: any, index: number) => (
