...
export default function AddThesisForm({ open, defaultOpen, className = "", onClose = () => { }, onSuccess = () => { } }: { open?: boolean, defaultOpen?: boolean, className?: string, onClose?: () => void, onSuccess?: () => void }) {
  ...
  const [thesisCourse, setThesisCourse] = React.useState<Courses|string>(Courses.BSIT);
  ...
  >
    <div className={
      clsx(
        "relative max-w-[1000px] min-w-[400px] bg-white text-black pt-4 px-4 rounded-lg border border-[#45515F] z-50",
        className
      )
    }>
      <form onSubmit={onSubmit}>
        <div className="grid grid-cols-2 justify-center items-center gap-3">
          <Input labelColor="text-black" className="text-black w-full" label="Thesis Title" name="title" value={thesisTitle} onChange={(e: any) => setThesisTitle(e.target.value)} disabled={isFormDisabled} required />
          <Input labelColor="text-black" className="text-black w-full" label="Author/s" name="author" value={thesisAuthor} onChange={(e: any) => setThesisAuthor(e.target.value)} disabled={isFormDisabled} required />
          <Select labelColor="text-black" className="text-black w-full" items={yearsList} label="Year" name="year" value={thesisYear} onChange={(e: any) => setThesisYear(e.target.value)} disabled={isFormDisabled} required />
          <Input labelColor="text-black" className="text-black w-full" label="Adviser" name="adviser" value={thesisAdviser} onChange={(e: any) => setThesisAdviser(e.target.value)} disabled={isFormDisabled} required />
          <Select labelColor="text-black" className="text-black w-full col-span-2" items={[{ label: "-- Select Department --", value: "" }, ...departmentList]} label="Department" name="department" value={thesisDepartment} onChange={(e: any) => { setThesisDepartment(e.target.value); setThesisCourse(""); }} disabled={isFormDisabled} required />
          <Select labelColor="text-black" className="text-black w-full col-span-2" items={[{ label: "-- Select Course --", value: "" }, ...courseList]} label="Course" name="course" value={thesisCourse} onChange={(e: any) => setThesisCourse(e.target.value)} disabled={isFormDisabled} required />
          <TextArea labelColor="text-black" className="text-black w-full col-span-2" label="Abstract" rows={3} name="abstract" value={thesisAbstract} onChange={(e: any) => setThesisAbstract(e.target.value)} disabled={isFormDisabled} required />
        </div>
