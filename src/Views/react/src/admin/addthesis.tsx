...
export default import(pathname("/jsx/imports")).then(async ({ React, Sweetalert2, clsx, getAsyncImport }) => {
  ...
  return function AddThesisForm({ open, defaultOpen, className = "", onClose = () => { }, onSuccess = () => { } }: { open?: boolean, defaultOpen?: boolean, className?: string, onClose?: () => void, onSuccess?: () => void }) {
    ...
    const courseList = React.useMemo(() => DepartmentCourses[thesisDepartment].map((d: any) => ({ label: d, value: d })), [thesisDepartment])
