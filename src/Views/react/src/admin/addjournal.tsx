...
import { React, ReactDOMClient, Sweetalert2, clsx, pathname } from '../imports'
...
export default function AddJournalForm({ open, defaultOpen, className = "", onClose = () => { }, onSuccess = () => { } }: { open?: boolean, defaultOpen?: boolean, className?: string, onClose?: () => void, onSuccess?: () => void }) {
  const { departments: departmentsAndCourses } = useDepartments();
  ...
  const [journalCourse, setJournalCourse] = React.useState<Courses|string>(Courses.BSIT);
  ...
  const courseList = React.useMemo(() => departmentsAndCourses[journalDepartment].map((d: any) => ({ label: d, value: d })), [journalDepartment])
  ...
  return ReactDOMClient.createPortal(
    <div className={
      clsx(
        "fixed left-0 top-0 w-full h-screen bg-gray-500/50 flex justify-center items-center z-40",
        show ? "block" : "hidden",
      )}
    >
      <div className={
        clsx(
          "max-w-[1000px] min-w-[400px] bg-white text-black pt-4 px-4 rounded-lg border border-[#45515F] z-50",
          className
        )
      }>
      <form onSubmit={onSubmit}>
        <div className="grid grid-cols-2 justify-center items-center gap-3">
          <Input labelColor="text-black" className="text-black w-full" label="Journal Title" name="title" value={journalTitle} onChange={(e: any) => setJournalTitle(e.target.value)} disabled={isFormDisabled} required />
          <Input labelColor="text-black" className="text-black w-full" label="Author/s" name="author" value={journalAuthor} onChange={(e: any) => setJournalAuthor(e.target.value)} disabled={isFormDisabled} required />
          <Select labelColor="text-black" className="text-black w-full" items={yearsList} label="Year" name="year" value={journalYear} onChange={(e: any) => setJournalYear(e.target.value)} disabled={isFormDisabled} required />
          <Input labelColor="text-black" className="text-black w-full" type="date" label="Published Date" name="author" value={journalPublishedDate} onChange={(e: any) => setJournalPublishedDate(e.target.value)} disabled={isFormDisabled} required />
          <Select labelColor="text-black" className="text-black w-full col-span-2" items={[{ label: "-- Select Department --", value: "" }, ...departmentList]} label="Department" name="department" value={journalDepartment} onChange={(e: any) => { setJournalDepartment(e.target.value); setJournalCourse(""); }} disabled={isFormDisabled} required />
          <Select labelColor="text-black" className="text-black w-full col-span-2" items={[{ label: "-- Select Course --", value: "" }, ...courseList]} label="Course" name="course" value={journalCourse} onChange={(e: any) => setJournalCourse(e.target.value)} disabled={isFormDisabled} required />
          <TextArea labelColor="text-black" className="text-black w-full col-span-2" label="Abstract" rows={3} name="abstract" value={journalAbstract} onChange={(e: any) => setJournalAbstract(e.target.value)} disabled={isFormDisabled} required />
        </div>
        <div className="flex items-center justify-center w-full px-4 mt-4 text-black">
          ...
          {!!pdf && (
            <div className={"border border-gray-500 border-dashed p-4 rounded-lg w-full h-32 flex items-center justify-center"}>
              <div className="flex justify-between border border-black p-3 rounded gap-x-3 items-center">
                <span className="text-black material-symbols-outlined">upload_file</span>
                <button type="button" onClick={openViewPdf}>
                  <span className="text-md text-black">{pdf.name}</span>
                </button>
                <div>
